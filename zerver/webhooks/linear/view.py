...
def get_issue_created_or_updated_message(event: str, payload: WildValue, action: str) -> str:
    ...
    priorityLabel = payload["data"]["priorityLabel"].tame(check_string)
    if priorityLabel != "No priority":
        to_add.append(f"Priority: {priorityLabel}")
...
def api_linear_webhook(
    ...
    subject = get_subject(user_specified_topic, event_type, payload)
...
def get_subject(user_specified_topic: Optional[str], event: str, payload: WildValue) -> str:
...
def get_event_type(payload: WildValue) -> Optional[str]:
    ...
    if type == "Issue":
        ...
        return "comment"
    elif type in IGNORED_EVENTS:
        return None

    complete_event = "{}:{}".format(
        type, payload.get("action", "???").tame(check_string)
    )  # nocoverage
